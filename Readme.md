...
The updated commands to run the components are:

```bash
cargo r --bin op-move --features storage,op-upgrade --release -- --genesis.l2-contract-genesis genesis.json --auth.jwt-secret jwt.txt
```
...
op-node \
  ...
  --rollup.l1-chain-config l1_genesis.json # Only this line new
```
...
op-batcher \
  ...
  --throttle.unsafe-da-bytes-lower-threshold 0 # Only this line is new
```
...
op-proposer \
  ...
  --game-factory-address $GAME_FACTORY_ADDRESS \ # This replaces L2OO
  --game-type 1 \ # Permissioned dispute games
  --proposal-interval 1s \ # This is needed if using game factory
  --wait-node-sync true # Not needed, but useful at sync
```
