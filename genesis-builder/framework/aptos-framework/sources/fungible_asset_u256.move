...
module aptos_framework::fungible_asset_u256 {
    ...
    ///     (width of the field - MAX_U256 is the implicit maximum supply)
    ///     if option::some(MAX_U256) is used, it is treated as unlimited supply.
    ...
    public fun supply<T: key>(metadata: Object<T>): Option<u256> acquires Supply {
    ...
    /// If supply is unlimited (or set explicitly to MAX_U256), none is returned
    public fun maximum<T: key>(metadata: Object<T>): Option<u256> acquires Supply {
        let metadata_address = object::object_address(&metadata);
        if (exists<Supply>(metadata_address)) {
            let supply = borrow_global<Supply>(metadata_address);
            supply.maximum
    ...
    public fun amount(fa: &FungibleAsset): u256 {
        fa.amount
    }

    #[view]
    /// Get the balance of a given store.
    public fun balance<T: key>(store: Object<T>): u256 acquires FungibleStore {
    ...
    public fun is_balance_at_least<T: key>(store: Object<T>, amount: u256): bool acquires FungibleStore {
        let store_addr = object::object_address(&store);
        is_address_balance_at_least(store_addr, amount)
    }

    /// Check whether the balance of a given store is >= `amount`.
    public(friend) fun is_address_balance_at_least(store_addr: address, amount: u256): bool acquires FungibleStore {
        if (store_exists_inline(store_addr)) {
    ...
    public fun mint(ref: &MintRef, amount: u256): FungibleAsset acquires Supply {
    ...
    public fun mint_to<T: key>(ref: &MintRef, store: Object<T>, amount: u256)
    ...
    public fun extract(fungible_asset: &mut FungibleAsset, amount: u256): FungibleAsset {
    ...
    fun increase_supply<T: key>(metadata: &Object<T>, amount: u256) acquires Supply {
        if (amount == 0) {
            return
        };
        let metadata_address = object::object_address(metadata);
    ...
    fun decrease_supply<T: key>(metadata: &Object<T>, amount: u256) acquires Supply {
        if (amount == 0) {
            return
        };
        let metadata_address = object::object_address(metadata);
