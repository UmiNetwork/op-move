...
module aptos_framework::primary_fungible_store_u256 {
    use aptos_framework::object::{Self, Object, ConstructorRef, DeriveRef};
    use aptos_framework::fungible_asset_u256::{Self, FungibleAsset, FungibleStore, Metadata, MintRef, TransferRef, BurnRef};
    ...
    public fun create_primary_store_enabled_fungible_asset(
        ...
        fungible_asset_u256::add_fungibility(
            constructor_ref,
            maximum_supply,
            name,
            symbol,
    ...
    public fun ensure_primary_store_exists<T: key>(
        ...
        if (fungible_asset_u256::store_exists(store_addr)) {
    ...
    public fun create_primary_store<T: key>(
        ...
        fungible_asset_u256::create_store(constructor_ref, metadata)
    }
    ...
    public fun primary_store_exists<T: key>(account: address, metadata: Object<T>): bool {
        fungible_asset_u256::store_exists(primary_store_address(account, metadata))
    }

    #[view]
    /// Get the balance of `account`'s primary store.
    public fun balance<T: key>(account: address, metadata: Object<T>): u256 {
        if (primary_store_exists(account, metadata)) {
            fungible_asset_u256::balance(primary_store(account, metadata))
        } else {
            0
        }
    }
    ...
    #[view]
    public fun is_balance_at_least<T: key>(account: address, metadata: Object<T>, amount: u256): bool {
        if (primary_store_exists(account, metadata)) {
            fungible_asset_u256::is_balance_at_least(primary_store(account, metadata), amount)
        } else {
            amount == 0
        }
    }
    ...
    public fun is_frozen<T: key>(account: address, metadata: Object<T>): bool {
        if (primary_store_exists(account, metadata)) {
            fungible_asset_u256::is_frozen(primary_store(account, metadata))
        } else {
            false
        }
    }
    ...
    public fun withdraw<T: key>(owner: &signer, metadata: Object<T>, amount: u256): FungibleAsset acquires DeriveRefPod {
        ...
        fungible_asset_u256::withdraw(owner, store, amount)
    }
    ...
    public fun deposit(owner: address, fa: FungibleAsset) acquires DeriveRefPod {
        let metadata = fungible_asset_u256::asset_metadata(&fa);
        let store = ensure_primary_store_exists(owner, metadata);
        fungible_asset_u256::deposit(store, fa);
    }
    ...
    public(friend) fun force_deposit(owner: address, fa: FungibleAsset) acquires DeriveRefPod {
        let metadata = fungible_asset_u256::asset_metadata(&fa);
        let store = ensure_primary_store_exists(owner, metadata);
        fungible_asset_u256::deposit_internal(object::object_address(&store), fa);
    }
    ...
    public entry fun transfer<T: key>(
        ...
        fungible_asset_u256::transfer(sender, sender_store, recipient_store, amount);
    }
    ...
    public entry fun transfer_assert_minimum_deposit<T: key>(
        ...
        may_be_unburn(sender, sender_store);
        let recipient_store = ensure_primary_store_exists(recipient, metadata);
        let start = fungible_asset_u256::balance(metadata);
        fungible_asset_u256::transfer(sender, sender_store, recipient_store, amount);
        let end = fungible_asset_u256::balance(metadata);
        assert!(end - start >= expected, error::aborted(EAMOUNT_MISMATCH));
    }

    /// Mint to the primary store of `owner`.
    public fun mint(mint_ref: &MintRef, owner: address, amount: u256) acquires DeriveRefPod {
        let primary_store = ensure_primary_store_exists(owner, fungible_asset_u256::mint_ref_metadata(mint_ref));
        fungible_asset_u256::mint_to(mint_ref, primary_store, amount);
    }

    /// Burn from the primary store of `owner`.
    public fun burn(burn_ref: &BurnRef, owner: address, amount: u256) {
        let primary_store = primary_store(owner, fungible_asset_u256::burn_ref_metadata(burn_ref));
        fungible_asset_u256::burn_from(burn_ref, primary_store, amount);
    }
    ...
    public fun set_frozen_flag(transfer_ref: &TransferRef, owner: address, frozen: bool) acquires DeriveRefPod {
        let primary_store = ensure_primary_store_exists(owner, fungible_asset_u256::transfer_ref_metadata(transfer_ref));
        fungible_asset_u256::set_frozen_flag(transfer_ref, primary_store, frozen);
    }

    /// Withdraw from the primary store of `owner` ignoring frozen flag.
    public fun withdraw_with_ref(transfer_ref: &TransferRef, owner: address, amount: u256): FungibleAsset {
        let from_primary_store = primary_store(owner, fungible_asset_u256::transfer_ref_metadata(transfer_ref));
        fungible_asset_u256::withdraw_with_ref(transfer_ref, from_primary_store, amount)
    }
    ...
    public fun deposit_with_ref(transfer_ref: &TransferRef, owner: address, fa: FungibleAsset) acquires DeriveRefPod {
        let from_primary_store = ensure_primary_store_exists(
            owner,
            fungible_asset_u256::transfer_ref_metadata(transfer_ref)
        );
        fungible_asset_u256::deposit_with_ref(transfer_ref, from_primary_store, fa);
    }
    ...
    public fun transfer_with_ref(
        ...
        let from_primary_store = primary_store(from, fungible_asset_u256::transfer_ref_metadata(transfer_ref));
        let to_primary_store = ensure_primary_store_exists(to, fungible_asset_u256::transfer_ref_metadata(transfer_ref));
        fungible_asset_u256::transfer_with_ref(transfer_ref, from_primary_store, to_primary_store, amount);
    }
